...
export default function CameraFeed() {
  ...
  const [showRemoveSpectaclesPrompt, setShowRemoveSpectaclesPrompt] =
    useState(false);
  ...
  const capturePhoto = () => {
    ...
    if (!isValidFace) {
      setError(
        "Please remove obstructions and ensure only one face is visible"
      );
      return;
    }
  ...
  useEffect(() => {
    ...
    faceMesh.onResults((results) => {
      ...
      //     alert("Multiple faces detected or occlusion occured.");
      ...
      if (results.multiFaceLandmarks?.length > 0) {
        ...
        // Obstruction check
        if (checkFaceObstructions(landmarks)) {
          setError("Remove hands/objects from your face");
          setIsValidFace(false);
        }
        ...
        setAvgLuminance(luminance); // You need to declare this state
  ...
  return (
    <div className="camera-container">
        ...
        {/* {autoCaptureMode && (
          <label>
            ...
            <input
              ...
              max="5"
              value={captureDelay}
              onChange={(e) =>setCaptureDelay(Math.max(1, Math.min(10, parseInt(e.target.value) || 2)))}
            />
          </label>
        )} */}
      </div>
      ...
      <div className="orientation-feedback">
        ...
        <p style={{ color: "red", fontSize: "135%" }}>
          ...
          {isAligned
            ? isCoolingDown
              ? "Hold on saving..."
              : autoCaptureMode
              ? ` Hold this position - Capturing in ${
        ...
        {/* <p style={{ color: avgLuminance < 100 ? "orange" : "green" }}>
          Lighting: {avgLuminance} / 255
        </p> */}
      ...
      {/* {!showRemoveSpectaclesPrompt &&
        !allPhotosCompleted && (
          <button
            ...
            disabled={!isAligned || !isLightingGood || !isWhiteBg}
            className={isAligned && !isLightingGood? "active-capture" : ""}
          >
          {isRetakeMode ? "Retake Photo" : `Capture ${ORIENTATION_SEQUENCE[currentOrientation].name}`}
              {wearsSpectacles &&
                ` (${
                  capturePhase === "withSpectacles" ? "With" : "Without"
                } Spectacles)`}
